...
class FabricList extends ControllerBase
{
    public function content()
    {
        ...
        if( isset($_POST['type']) ) {
            if($_POST['type'] == 'Delete' && isset($_POST['id'])){
                $d = [];
                $r = SofaApiLogic::send('Fabrics', $d, 'DELETE', $_POST['id']);
                if($r === []) {
					SofaApiLogic::my_goto("Удалена ткань: {$_POST['id']}");
                } else {
        ...
        foreach ($result['values'] as $item){
            ...
            $image_preview_url =  $item['textures']['main_texture']['previewUrl'] ?? NULL;
            $playerUrl = '<a href="/fabric/'.$item['id'].'/info" class="btn btn-default col-xs-12 glyphicon glyphicon-list-alt"> Подробнее</a>';
            ...
            $deleteAction = [
                ...
                'action' => ['#type'=>'submit'   , '#value' => 'Удалить', '#attributes' => [
                    'onclick' => 'if(!confirm("Удалить ткань?")){return false;}',
