...
class CollectionList extends ControllerBase
{
    public function content()
    {
        ...
        if( isset($_POST['type']) ) {
            if($_POST['type'] == 'Delete' && isset($_POST['id'])){
                $d = [];
                $r = SofaApiLogic::send('Collections', $d, 'DELETE', $_POST['id']);
                if($r === []) {
					SofaApiLogic::my_goto("Удалена коллекцию: {$_POST['id']}");
                } else {
        ...
        foreach ($result['values'] as $item){
            ...
            $image_preview_url =  $item['images']['main_texture']['previewUrl'] ?? NULL;
            $playerUrl = '<a href="/collection/'.$item['id'].'/edit" class="btn btn-default col-xs-12 glyphicon glyphicon-list-alt"> Подробнее</a>';
            ...
            $deleteAction = [
                ...
                'action' => ['#type'=>'submit'   , '#value' => 'Удалить', '#attributes' => [
                    'onclick' => 'if(!confirm("Удалить коллекцию?")){return false;}',
                    'class'=>['col-xs-12', 'btn-danger'], ], ],
            ];
			

			if($image_preview_url != NULL){
				$uri = "http://souz-m.tk".$image_preview_url;
				$avatar_url = "<img src=\"{$uri}\" width=\"203px\" height=\"168px\">";
			}
