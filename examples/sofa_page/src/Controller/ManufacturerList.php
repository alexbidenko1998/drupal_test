...
class ManufacturerList extends ControllerBase
{
    public function content()
    {
        ...
        if( isset($_POST['type']) ) {
            if($_POST['type'] == 'Delete' && isset($_POST['id'])){
                $d = [];
                $r = SofaApiLogic::send('Manufacturers', $d, 'DELETE', $_POST['id']);
                if($r === []) {
					SofaApiLogic::my_goto("Удален производитель: {$_POST['id']}");
                } else {
        ...
        foreach ($result['values'] as $item){
            ...
            $playerUrl = '<a href="/manufacturer/'.$item['id'].'/edit" class="btn btn-default col-xs-12 glyphicon glyphicon-list-alt"> Подробнее</a>';
            ...
            $deleteAction = [
                ...
                'action' => ['#type'=>'submit'   , '#value' => 'Удалить', '#attributes' => [
                    'onclick' => 'if(!confirm("Удалить производителя?")){return false;}',
