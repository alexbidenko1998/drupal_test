...
class SofaAddModel extends FormBase {
    /**
     * {@inheritdoc}.
     */
    // Метод для котороый возвращает ид формы.
    public function getFormId() {
        return 'SofaAddModel_form';
    }
    ...
    public function buildForm(array $form, FormStateInterface $form_state, $sofaId = '') {
        ...
        $form['formfile'] = array(
            ...
            '#upload_location' => 'public://model_file/',
            '#required' => TRUE,
        );
...
/*
            $regex = '/\\.(' . preg_replace('/ +/', '|', preg_quote('png')) . ')$/i';
            if (!preg_match($regex, $fileUrl)) {
                $form_state->setErrorByName('formfile', $this-> t('Only files with the following extensions are allowed: png'));
            }
        }*/
    }
    ...
    public function submitForm(array &$form, FormStateInterface $form_state) {
		...
		$result = SofaApiLogic::send($method, $data, 'POST_MULT', $id.'/model_file', 'array');
        if($result === []){
